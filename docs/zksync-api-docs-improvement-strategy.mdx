...
# Sksync's API Docs Improvement Approach

### Who reads Zksync's technical documentation?
Let's posit that there are currently two main readership profiles for Zksync API docs:
- **Architects/CTOs**
- **Developers**
...
- **Developers** need all of the above plus API SDKs, playgrounds, and more sample code. They also need comprehensive/updated API references to avoid guessing their way through the docs and documents tailored to their use cases (i.e., designing an NFT market's front end that consumes Zksync's data).

## Assessment

Here is a partial/subjective assessment of Zksync's API documentation's current state:

### Pros

Zksync technical documentation: 
- Covers core concepts.
- Makes it possible to interact quickly with Zksync's API.
For instance: the API context, its endpoints, and the expected responses are clearly explained, often in plain, clear English.
...
- Information can be scattered across various pages, making it sometimes hard to make sense out of the content.
...
- An API documentation framework (redocly, spotlight) so the APIs can be covered extensively, with proposed code samples and Postman collections.
